...
def userWelcome(request):
    ...
    for appraisment in objappraisment:
        ...
        if appraisment.status=='Initial':
            ...
            elif appraisment.status=='Completed' or appraisment.status=='Reports':
                status='Submitted'
                scolor='background: rgb(232,34,54); /* Old browsers */ background: -moz-linear-gradient(-45deg, rgba(232,34,54,1) 0%, rgba(216,22,41,1) 0%, rgba(190,18,35,1) 100%); /* FF3.6+ */ background: -webkit-gradient(linear, left top, right bottom, color-stop(0%,rgba(232,34,54,1)), color-stop(0%,rgba(216,22,41,1)), color-stop(100%,rgba(190,18,35,1))); /* Chrome,Safari4+ */ background: -webkit-linear-gradient(-45deg, rgba(232,34,54,1) 0%,rgba(216,22,41,1) 0%,rgba(190,18,35,1) 100%); /* Chrome10+,Safari5.1+ */ background: -o-linear-gradient(-45deg, rgba(232,34,54,1) 0%,rgba(216,22,41,1) 0%,rgba(190,18,35,1) 100%); /* Opera 11.10+ */ background: -ms-linear-gradient(-45deg, rgba(232,34,54,1) 0%,rgba(216,22,41,1) 0%,rgba(190,18,35,1) 100%); /* IE10+ */ background: linear-gradient(135deg, rgba(232,34,54,1) 0%,rgba(216,22,41,1) 0%,rgba(190,18,35,1) 100%); /* W3C */ filter: progid:DXImageTransform.Microsoft.gradient( startColorstr=\'#e82236\', endColorstr=\'#be1223\',GradientType=1 ); /* IE6-9 fallback on horizontal gradient */'
    ...
    if request.method == 'POST' :
        ...
        try:
            ...
            if (int(i_totalQuestionCount) != 0 and int(answeredcount) != 0) and int(i_totalQuestionCount) == int(answeredcount) :
                ...
                response['message'] = 'Appraisal submitted for ' + userAppraised.firstname + ' ' + userAppraised.lastname
